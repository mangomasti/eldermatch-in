...
function buildReal(sp: RealFacilitySpec, idx: number): Facility {
  ...
  return {
    ...
    longDescription: `${sp.longDescription}${sp.listingNote ? ` Note: ${sp.listingNote}` : ""} Pricing: ${sp.priceNote}. Source of listing information: ${sp.source}${sp.sourceType === "secondary" ? " (secondary sources — not confirmed by the facility)" : ""}. This listing is unverified — details are drawn from public sources and have not yet been confirmed by the facility.`,
    images: [pick(0), pick(1), pick(2), pick(3)],
    photosPending: true,
    careTypes: sp.careTypes,
    amenities: sp.amenities,
    medicalCapabilities: sp.medicalCapabilities.length ? sp.medicalCapabilities : sp.conditionCare,
    staffRatio: sp.staffRatio,
    licensing: sp.licensing,
    languages: sp.languages,
    residentInterests: sp.residentInterests,
    schedule: [],
...
export const INDIAN_STATES = Array.from(new Set(facilities.map((f) => f.state))).sort();
